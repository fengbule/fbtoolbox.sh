  ...
  while true; do
    ...
    case "$n" in
      ...
      5)
        ...
        pause_enter
        ;;
      6) handle_item "Windows 激活命令" "powershell -c \"irm https://get.activated.win | iex\"" ;;
      0) return 0 ;;
      *) warn "无效选择"; pause_enter ;;
    esac
  done
...
menu_benchmark_all() {
  while true; do
    ...
    case "$n" in
      ...
      3) handle_item "融合怪 ecs.sh" "bash <(wget -qO- --no-check-certificate https://gitlab.com/spiritysdx/za/-/raw/main/ecs.sh)" ;;
...
menu_perf() {
  while true; do
    ...
    case "$n" in
      ...
      2) handle_item "跳过网络，测 GB5" "curl -sL yabs.sh | bash -s -- -i5" ;;
      3) handle_item "跳过网络和磁盘，测 GB5" "curl -sL yabs.sh | bash -s -- -if5" ;;
      4) handle_item "改测 GB5 不测 GB6" "curl -sL yabs.sh | bash -s -- -5" ;;
      0) return 0 ;;
      *) warn "无效选择"; pause_enter ;;
    esac
  done
...
menu_media() {
  while true; do
    ...
    n="${n:-1}"
    case "$n" in
      1) handle_item "最常用版本" "bash <(curl -L -s check.unlock.media)" ;;
      2) handle_item "原生检测脚本" "bash <(curl -sL Media.Check.Place)" ;;
      3) handle_item "准确度最高" "bash <(curl -L -s https://github.com/1-stream/RegionRestrictionCheck/raw/main/check.sh)" ;;
      4) handle_item "IP 质量体检脚本" "bash <(curl -sL IP.Check.Place)" ;;
      5) handle_item "一键修改解锁 DNS" "wget https://raw.githubusercontent.com/Jimmyzxk/DNS-Alice-Unlock/refs/heads/main/dns-unlock.sh && bash dns-unlock.sh" ;;
      6)
        echo
        echo "安装别名："
...
menu_speedtest() {
  while true; do
    ...
    case "$n" in
      ...
      5) handle_item "区域速度测试" "wget -qO- nws.sh | bash -s -- -r region_name" ;;
...
menu_backtrace() {
  while true; do
    ...
    n="${n:-1}"
    case "$n" in
      1) handle_item "直接显示回程（小白）" "curl https://raw.githubusercontent.com/ludashi2020/backtrace/main/install.sh -sSf | sh" ;;
      2) handle_item "回程详细测试 AutoTrace" "wget -N --no-check-certificate https://raw.githubusercontent.com/Chennhaoo/Shell_Bash/master/AutoTrace.sh && chmod +x AutoTrace.sh && bash AutoTrace.sh" ;;
...
menu_functions() {
  while true; do
    ...
    case "$n" in
      ...
      3) handle_item "一键开启 BBR" "echo 'net.core.default_qdisc=fq' >> /etc/sysctl.conf && echo 'net.ipv4.tcp_congestion_control=bbr' >> /etc/sysctl.conf && sysctl -p && sysctl net.ipv4.tcp_available_congestion_control && lsmod | grep bbr" "danger" ;;
      4) handle_item "多功能 BBR 安装脚本" "wget -N --no-check-certificate 'https://gist.github.com/zeruns/a0ec603f20d1b86de6a774a8ba27588f/raw/4f9957ae23f5efb2bb7c57a198ae2cffebfb1c56/tcp.sh' && chmod +x tcp.sh && ./tcp.sh" ;;
      ...
      8) handle_item "25 端口开放测试" "telnet smtp.aol.com 25" ;;
      0) return 0 ;;
      *) warn "无效选择"; pause_enter ;;
    esac
  done
...
menu_dufu() {
  while true; do
    ...
    case "$n" in
      ...
      5) handle_item "InstallNET Debian 12 RAID0" "wget --no-check-certificate -qO InstallNET.sh 'https://raw.githubusercontent.com/leitbogioro/Tools/master/Linux_reinstall/InstallNET.sh' && chmod a+x InstallNET.sh && bash InstallNET.sh -debian 12 -raid '0'" "danger" ;;
      6) handle_item "InstallNET 指定网络 DD" "wget --no-check-certificate -qO InstallNET.sh 'https://raw.githubusercontent.com/leitbogioro/Tools/master/Linux_reinstall/InstallNET.sh' && chmod a+x InstallNET.sh && bash InstallNET.sh -debian 12 --ip-addr 139.162.52.1 --ip-mask 24 --ip-gate 139.162.52.1 --ip6-addr 2a07:e040:2:1d3::1 --ip6-gate 2a07:e040::1 --ip6-mask 32" "danger" ;;
      7) handle_item "InstallNET 指定密码 DD" "wget --no-check-certificate -qO InstallNET.sh 'https://raw.githubusercontent.com/leitbogioro/Tools/master/Linux_reinstall/InstallNET.sh' && chmod a+x InstallNET.sh && bash InstallNET.sh -debian 12 -pwd 'password'" "danger" ;;
